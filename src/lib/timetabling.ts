...
export function gerarHorarioAlgoritmico(
  ...
  const MAX_ATTEMPTS = 100; // Aumentado para 100 tentativas conforme solicitado
  ...
  // 3. Função Auxiliar para Criar Blocos de Aulas
  const criarBlocos = (total: number, compId: string) => {
    ...
    if (!config || !config.geminar || config.tamanho_bloco <= 1) {
        return Array(total).fill(1); // Lista de blocos de tamanho 1
    }
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    const ocupacaoProfessores = new Set<string>(); // "dia-aulaIdx-profId"
    ...
    // Ordenar blocos: maiores primeiro (são mais difíceis de encaixar)
    ...
    for (const b of blocosPresenciais) {
        let alocado = false;
        // Tentamos slots aleatórios primeiro para espalhar a carga
        ...
        for (const slot of slotsTentativa) {
            ...
            for (let k = 0; k < b.size; k++) {
                ...
                if (b.professor_id) {
                    if (ocupacaoProfessores.has(`${d}-${idx}-${b.professor_id}`)) { livre = false; break; }
            ...
            if (livre) {
                for (let k = 0; k < b.size; k++) {
                    ...
                    if (b.professor_id) ocupacaoProfessores.add(`${d}-${idx}-${b.professor_id}`);
                }
                alocado = true;
                break;
            }
        ...
        for (const slot of slotsTentativa) {
            ...
            for (let k = 0; k < b.size; k++) {
                ...
                if (b.professor_id && turnoOposto) {
                    if (ocupacaoProfessores.has(`${d}-OP-${idx}-${b.professor_id}`)) { livre = false; break; }
                    const prof = professores.find(pr => pr.id === b.professor_id);
            ...
            if (livre) {
                for (let k = 0; k < b.size; k++) {
                    ...
                    if (b.professor_id) ocupacaoProfessores.add(`${d}-OP-${idx}-${b.professor_id}`);
                }
                alocado = true;
                break;
            }
  ...
  agrupado.forEach(p => {
    ...
    diagnostico.push(`• Turma ${p.turma_nome}: "${p.componente_nome}" (${p.totalAulas} aulas pendentes).\n  SUGESTÃO: O professor ${prof?.nome_horario || 'especificado'} pode estar sem janelas livres. Remova folgas dele ou reduza restrições na Série.`);
  });
