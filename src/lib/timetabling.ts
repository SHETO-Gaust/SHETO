...
/**
 * Algoritmo de Timetabling Determinístico com Diagnóstico de Erros
 */
export function gerarHorarioAlgoritmico(
  ...
  if (pendenciasFaltandoProfessor.length > 0) {
    return {
      success: false,
      aulas: [],
  ...
  for (const dia of diasAtivos) {
    for (let aulaIdx = 0; aulaIdx < numAulas; aulaIdx++) {
      for (const turma of turmas) {
        // Verifica se a turma já tem aula neste slot (pode acontecer se o loop for por turma primeiro)
        ...
        const indexPendente = presenciaisOrdenadas.findIndex(p => {
          ...
          if (p.professor_id) {
            ...
            // Choque de horário (professor já em outra turma)
            if (ocupacaoProfessores.has(`${dia}-${aulaIdx}-${p.professor_id}`)) return false;
            // Indisponibilidade marcada pelo professor
  ...
  if (presenciaisOrdenadas.length > 0 || npOrdenadas.length > 0) {
    ...
    // Agrupar por Turma + Componente para análise
    ...
    agrupado.forEach((val) => {
        ...
        if (prof) {
            // Contar quantos slots o professor tem livre na semana para este turno
            ...
            if (slotsLivresProfessor < val.count) {
                motivo = `O professor ${prof.nome_horario} tem apenas ${slotsLivresProfessor} horários disponíveis, mas precisa de mais aulas.`;
            } else {
                motivo = `O professor ${prof.nome_horario} está ocupado em outras turmas nos horários em que esta turma está livre.`;
            }
        }

        diagnostico.push(`• Turma ${val.turma}: "${val.comp}" (${val.count} aulas restantes). Motivo provável: ${motivo}`);
    });
