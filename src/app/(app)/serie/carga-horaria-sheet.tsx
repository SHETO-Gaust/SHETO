    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetContent onPointerDownOutside={(e) => e.preventDefault()} className="sm:max-w-2xl flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>Defina a quantidade de aulas de cada disciplina nesta série.</SheetDescription>
        </SheetHeader>
        ...
        <Form {...form}>
          <form id="carga-form" onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto pr-2 -mr-4 space-y-4 py-4">
              ...
              {/*
                O travamento de dia/horário morava aqui e valia para todas as turmas da
                série ao mesmo tempo. Agora é por turma, na tela de Turmas.
              */}
              <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/50 border rounded-md p-3">
                <Lock className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                <p>
                  Para travar uma disciplina em dia e horário fixos, use{' '}
                  <Link href="/turmas" className="font-semibold text-primary hover:underline">Turmas → Travar Aulas</Link>.
                  O travamento passou a ser por turma, e não mais para a série inteira.
                </p>
              </div>
              ...
              <div className="space-y-3">
                {fields.map((field, index) => {
                  ...
                  return (
                    <div key={field.id} className="border rounded-lg p-3 bg-card flex justify-between items-center">
