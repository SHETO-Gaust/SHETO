...
import { AvaliacaoSummaryCard } from './avaliacao-summary-card';

type AvaliacoesAdminClientProps = {
    allFormacoes: Pick<Formacao, 'id' | 'name'>[];
};

export function AvaliacoesAdminClient({ allFormacoes }: AvaliacoesAdminClientProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {formacoesToDisplay.length > 0 && (
                 <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
                    {formacoesToDisplay.map((formacao) => (
                        <AvaliacaoSummaryCard key={formacao.id} formacaoId={formacao.id} />
                    ))}
                </div>
            )}
        </div>
