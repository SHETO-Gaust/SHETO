...
async function getOpenFormacoes(): Promise<Formacao[]> {
    ...
    const { data, error } = await supabase
        .from('formacoes')
        .select('*')
        .filter('subscription_form_config->>open', 'eq', 'true')
        .order('created_at', { ascending: false });

    if (error) {
        console.error('Error fetching open formacoes:', error);
        return [];
    }
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-bold tracking-tight">Inscrições Abertas</h1>
        <p className="text-muted-foreground mt-2">
          Escolha uma das formações abaixo para realizar sua inscrição.
        </p>
      </div>
      ...
      {formacoes.length > 0 ? (
            ...
            <Card key={formacao.id} className="flex flex-col">
              ...
              <CardContent className="flex-grow">
                 <p className="text-sm text-muted-foreground">
                    Clique no botão abaixo para preencher o formulário de inscrição.
                 </p>
              </CardContent>
              <CardFooter>
                <Link href={`/inscricoes/${formacao.id}`} className="w-full">
        ...
        <div className="text-center text-muted-foreground border-2 border-dashed rounded-lg p-12">
            <p className="text-lg">Nenhuma inscrição aberta no momento.</p>
